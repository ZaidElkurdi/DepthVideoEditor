...
class RangeSeekSlider: UIControl {
  ...
  private func setup() {
    ...
    sliderLineBetweenHandles.backgroundColor = UIColor(red: 0.0, green: 124.0/255.0, blue: 246.0/255.0, alpha: 1.0).cgColor
    sliderLine.backgroundColor = UIColor.white.cgColor
    
    leftHandle.backgroundColor = UIColor.white.cgColor
    leftHandle.borderColor = leftHandle.backgroundColor
    
    rightHandle.backgroundColor = UIColor.white.cgColor
    rightHandle.borderColor = rightHandle.backgroundColor
  ...
  private func xPositionAlongLine(for value: CGFloat, handle: CALayer) -> CGFloat {
    ...
    if handle == leftHandle {
      return max(15.0, sliderLine.frame.minX + offset)
    } else {
      return min(maxMinDif - 15.0, sliderLine.frame.minX + offset)
    }
  }
  
  private func updateLineHeight() {
    let barSidePadding: CGFloat = 0
    let yMiddle: CGFloat = 15.0
  ...
  private func updateLabelPositions() {
    ...
    let newMinLabelCenter: CGPoint = CGPoint(x: leftHandle.frame.midX,
                                             y: leftHandle.frame.maxY + (minLabelTextSize.height / 2.0) + labelPadding)
    
    let newMaxLabelCenter: CGPoint = CGPoint(x: rightHandle.frame.midX,
                                             y: rightHandle.frame.maxY + (maxLabelTextSize.height / 2.0) + labelPadding)
...
rangeSlider.frame = CGRect(x: 0, y: 0, width: 300, height: 55.0)

let containerView = UIView(frame: CGRect(x: 0, y: 0, width: 500, height: 100))
